...
(function ($) {
    ...
    function mousedownHandler(event) {
        ...
        let moveEventHandler = createMousemove($splitPane, pageXof(event), pageYof(event));
    ...
    function getComponentsSizes($splitPane) {
        let property = $splitPane.is('.fixed-top, .fixed-bottom, .horizontal-percent') ?
            'height' : 'width';
        return {
    ...
    function attachResizeHandler() {
        ...
        $(parent).on(parent === window ? 'resize' : 'splitpaneresize', function(event) {
            let target = event.target === document ? window : event.target;
            if (target === parent) {
    ...
    function createParentresizeHandler($splitPane) {
        let components = getComponents($splitPane);
        if ($splitPane.is('.fixed-top')) {
            return function() {
                let lastComponentMinHeight = minHeight(components.last),
            ...
            return function() {
                let firstComponentMinHeight = minHeight(components.first),
            ...
            return function() {
                let lastComponentMinWidth = minWidth(components.last),
            ...
            return function() {
                let firstComponentMinWidth = minWidth(components.first),
    ...
    function fixedTopHandler(components, pageY) {
        ...
        return function(event) {
            ...
            let top = newTop(firstComponentMinHeight, maxFirstComponentHeight, topOffset + pageYof(event));
    ...
    function fixedBottomHandler(components, pageY) {
        ...
        return function(event) {
            event.preventDefault && event.preventDefault();
            let bottom = Math.min(Math.max(lastComponentMinHeight, bottomOffset - pageYof(event)), maxLastComponentHeight);
    ...
    function horizontalPercentHandler(components, pageY) {
        let splitPaneHeight = components.splitPane.offsetHeight,
    ...
    function fixedLeftHandler(components, pageX) {
        ...
        return function(event) {
            event.preventDefault && event.preventDefault();
            let left = newLeft(firstComponentMinWidth, maxFirstComponentWidth, leftOffset + pageXof(event));
    ...
    function fixedRightHandler(components, pageX) {
        ...
        return function(event) {
            event.preventDefault && event.preventDefault();
            let right = Math.min(Math.max(lastComponentMinWidth, rightOffset - pageXof(event)), maxLastComponentWidth);
    ...
    function verticalPercentHandler(components, pageX) {
        let splitPaneWidth = components.splitPane.offsetWidth,
